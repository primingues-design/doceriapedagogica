...
export function PdfApostilaDoc({ dados, config, data }: Props) {
  // paddingBottom extra para o rodapé nunca sobrepor o conteúdo
  const pageStyle = {
    backgroundColor: '#ffffff',
    paddingTop: config.pagina.margemSuperior,
    paddingBottom: config.pagina.margemInferior + 24,
  ...
  const s = StyleSheet.create({
    ...
    capaTag: {
      ...
      fontSize: 10, fontFamily: config.tipografia.fonteCorpo, color: '#555555',
    },
    camposAluno: {
      borderWidth: 1.5, borderColor: '#cccccc', borderRadius: 6, flexDirection: 'row',
    },
    campoAluno: {
      flex: 1, paddingHorizontal: 12, paddingVertical: 9,
      borderRightWidth: 1, borderRightColor: '#cccccc',
      fontSize: 11, fontFamily: config.tipografia.fonteCorpo, color: '#333333',
    },

    // Seções
    secaoLabel: {
      fontSize: 8, fontFamily: config.tipografia.fonteTitulo,
      color: config.cores.primaria, textTransform: 'uppercase',
      letterSpacing: 1, marginBottom: 8,
      paddingBottom: 4, borderBottomWidth: 1, borderBottomColor: config.cores.destaque,
    },
    secaoTitulo: {
      fontSize: config.tipografia.tamanhoSubtitulo,
      fontFamily: config.tipografia.fonteTitulo,
      color: config.cores.primaria, marginBottom: 10,
    },
    paragrafo: {
      fontSize: config.tipografia.tamanhoCorpo, fontFamily: config.tipografia.fonteCorpo,
      color: '#000000', lineHeight: config.tipografia.alturaDeLinha, marginBottom: 8,
    },

    // Destaque
    destaqueWrap: {
      backgroundColor: '#fdf3e3', borderLeftWidth: 4, borderLeftColor: '#c8963e',
      borderRadius: 4, paddingHorizontal: 14, paddingVertical: 10, marginVertical: 12,
    },
    destaqueLabel: {
      fontSize: 8, fontFamily: config.tipografia.fonteTitulo, color: '#c8963e',
      textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 4,
    },
    destaqueTxt: {
      fontSize: 11, fontFamily: config.tipografia.fonteCorpo, color: '#4a4845', lineHeight: 1.6,
    },

    // Mapa mental
    mapaCentro: {
      alignSelf: 'center', backgroundColor: config.cores.primaria,
      paddingHorizontal: 16, paddingVertical: 7, borderRadius: 6, marginBottom: 10,
    },
    mapaCentroTxt: {
      fontSize: 12, fontFamily: config.tipografia.fonteTitulo, color: '#ffffff', textAlign: 'center',
    },
    mapaRamos: { flexDirection: 'row', flexWrap: 'wrap', gap: 6 },
    mapaRamo: {
      backgroundColor: config.cores.destaque, borderWidth: 1, borderColor: config.cores.primaria,
      borderRadius: 4, paddingHorizontal: 10, paddingVertical: 5,
    },
    mapaRamoTxt: { fontSize: 10, fontFamily: config.tipografia.fonteCorpo, color: config.cores.primaria },
    ...
    atividadeNum: {
      width: 24, height: 24, borderRadius: 12, backgroundColor: config.cores.primaria,
      justifyContent: 'center', alignItems: 'center',
    },
    atividadeNumTxt: { fontSize: 11, fontFamily: config.tipografia.fonteTitulo, color: '#ffffff' },
    atividadeTipo: {
      fontSize: 9, fontFamily: config.tipografia.fonteTitulo, color: config.cores.primaria,
      textTransform: 'uppercase', letterSpacing: 0.8,
    },
    atividadeQ: {
      fontSize: config.tipografia.tamanhoQuestao, fontFamily: config.tipografia.fonteCorpo,
      color: '#000000', lineHeight: 1.7, marginBottom: 10,
    },
    linhaResposta: {
      borderBottomWidth: 1.5, borderBottomColor: '#888888',
      marginBottom: config.espacamento.linhasResposta,
    },
    caixaResposta: {
      borderWidth: 1.5, borderColor: '#cccccc', borderRadius: 5, minHeight: 80, marginTop: 4,
    },
    opcaoWrap: { flexDirection: 'row', alignItems: 'flex-start', marginBottom: 6, gap: 8 },
    opcaoBolinha: {
      width: 20, height: 20, borderRadius: 10, borderWidth: 1.5, borderColor: '#555555',
      justifyContent: 'center', alignItems: 'center', marginTop: 1,
    },
    opcaoLetra: { fontSize: 10, fontFamily: config.tipografia.fonteTitulo, color: '#333333' },
    opcaoTxt: {
      fontSize: config.tipografia.tamanhoQuestao, fontFamily: config.tipografia.fonteCorpo,
      color: '#000000', flex: 1, lineHeight: 1.5,
    },
    vfItem: {
      flexDirection: 'row', alignItems: 'center', paddingVertical: 4,
      borderBottomWidth: 0.5, borderBottomColor: '#cccccc', gap: 10,
    },
    vfBoxes: { flexDirection: 'row', gap: 3 },
    vfBox: {
      width: 22, height: 22, borderWidth: 1.5, borderColor: '#555555',
    ...
    // Fechamento
    fechamentoWrap: {
      backgroundColor: '#e8f4f4', borderWidth: 2, borderColor: '#2d6e6e',
      borderRadius: 8, paddingHorizontal: 28, paddingVertical: 24, marginBottom: 16,
    },
    fechamentoLabel: {
      fontSize: 8, fontFamily: config.tipografia.fonteTitulo, color: '#4a9b9b',
      textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8,
    },
    fechamentoTitulo: {
      fontSize: config.tipografia.tamanhoSubtitulo, fontFamily: config.tipografia.fonteTitulo,
      color: '#2d6e6e', marginBottom: 10,
    },
    fechamentoTxt: {
      fontSize: config.tipografia.tamanhoCorpo, fontFamily: config.tipografia.fonteCorpo,
      color: '#2d6e6e', lineHeight: config.tipografia.alturaDeLinha, marginBottom: 6,
    },
    orientacaoWrap: {
      backgroundColor: '#f0eeff', borderLeftWidth: 4, borderLeftColor: '#6c63d4',
      borderRadius: 4, paddingHorizontal: 14, paddingVertical: 10, marginVertical: 10,
    },
    orientacaoLabel: {
      fontSize: 8, fontFamily: config.tipografia.fonteTitulo, color: '#6c63d4',
      textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 4,
    },
    orientacaoTxt: {
      fontSize: 10, fontFamily: config.tipografia.fonteCorpo, color: '#4a4880', lineHeight: 1.6,
    },
    pcWrap: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, marginTop: 10 },
    pcTag: {
      backgroundColor: '#f5f0e8', borderWidth: 1, borderColor: '#cccccc', borderRadius: 12,
      paddingHorizontal: 10, paddingVertical: 3,
      fontSize: 10, fontFamily: config.tipografia.fonteCorpo, color: '#8a8785',
    },
    musicaWrap: {
      backgroundColor: '#fdf3e3', borderRadius: 8, borderLeftWidth: 4,
      borderLeftColor: '#c8963e', paddingHorizontal: 20, paddingVertical: 16,
    },
    musicaVerso: {
      fontSize: config.tipografia.tamanhoCorpo, fontFamily: config.tipografia.fonteCorpo,
      color: '#4a4845', lineHeight: 2,
    },
  });

  function renderRodape(titulo: string) {
    return (
      <View style={s.rodape} fixed>
        <Text style={s.rodapeTxt}>Doceria Pedagógica</Text>
        <Text style={s.rodapeTxt}>{titulo}</Text>
        ...
        <View style={{ marginTop: 6 }}>
          {at.opcoes.map((op, oi) => (
            <View key={oi} style={s.opcaoWrap}>
              <View style={s.opcaoBolinha}><Text style={s.opcaoLetra}>{LETRAS_OPCOES[oi]}</Text></View>
    ...
    return (
      <View key={at.numero} style={s.atividadeWrap} wrap={false}>
        <View style={s.atividadeHeader}>
          <View style={s.atividadeNum}><Text style={s.atividadeNumTxt}>{at.numero}</Text></View>
  ...
  return (
    <Document>
      ...
      <Page size="A4" style={pageStyle}>
        <View style={{
          borderTopWidth: 5, borderTopColor: config.cores.primaria, paddingTop: 16, marginBottom: 24,
        }}>
      ...
      <Page size="A4" style={pageStyle}>
        ...
        {dados.mapa_mental && (
          <View style={{ marginTop: 16 }}>
            <Text style={[s.secaoLabel, { marginBottom: 12 }]}>Mapa Mental</Text>
      ...
      {/* ── PÁGINAS DE ATIVIDADES (uma Page por atividade = sem overflow) ── */}
      {dados.atividades.map((at, idx) => (
        <Page key={at.numero} size="A4" style={pageStyle}>
          {idx === 0 && (
      ...
      <Page size="A4" style={pageStyle}>
        ...
        {dados.palavras_chave?.length ? (
          <View>
            <Text style={[s.secaoLabel, { marginTop: 14 }]}>Palavras-chave</Text>
            <View style={s.pcWrap}>
